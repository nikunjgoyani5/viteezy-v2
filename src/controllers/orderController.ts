...
class OrderController {
  ...
  createOrder = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<any> => {
      ...
      const backendPricingSummary = await checkoutService.getCheckoutPageSummary(
        req.user._id.toString(),
        {
          sachets: sachetItems.length > 0 && selectedPlanDays
            ? { planDurationDays: selectedPlanDays as 30 | 60 | 90 | 180 }
            : undefined,
          standUpPouch: standupPouchItems.length > 0
            ? {
                capsuleCount: selectedCapsuleCount as 60 | 120,
                planDays: selectedStandUpPouchPlanDays as 60 | 120,
      ...
      const backendPricing = backendPricingSummary.data.pricing;
      const normalizedCurrency =
        backendPricing.overall.currency.toString().toUpperCase();

      if (
        pricing?.overall &&
        Math.abs(
          Number(pricing.overall.grandTotal || 0) -
            Number(backendPricing.overall.grandTotal || 0)
        ) > 0.01
      ) {
        logger.warn(
          `Ignoring frontend pricing mismatch for cart ${cartId}. Frontend grandTotal=${pricing.overall.grandTotal}, backend grandTotal=${backendPricing.overall.grandTotal}`
        );
      }

      const refreshedCart = await Carts.findOne({
        _id: new mongoose.Types.ObjectId(cartId),
      ...
      // Frontend pricing is no longer authoritative, but validate it if supplied
      // so obviously malformed payloads can still be diagnosed.
      if (pricing?.overall) {
        if (
      ...
      if (normalizedCouponCode) {
        try {
          const couponResult = await validateCouponForOrder({
            ...
            userId: req.user._id,
            orderAmount:
              backendPricing.overall.discountedPrice -
              backendPricing.overall.membershipDiscountAmount +
              backendPricing.overall.taxAmount,
            shippingAmount: 0,
      ...
      // Pricing breakdown is recalculated on the backend; frontend pricing is informational only.
      const pricingBreakdown: any = {
        ...backendPricing,
        overall: {
          ...backendPricing.overall,
          currency: normalizedCurrency,
        },
        ...(backendPricing.sachets
          ? {
              sachets: {
                ...backendPricing.sachets,
                currency: (backendPricing.sachets.currency || normalizedCurrency)
                  .toString()
                  .toUpperCase(),
              },
            }
          : {}),
        ...(backendPricing.standUpPouch
          ? {
              standUpPouch: {
                ...backendPricing.standUpPouch,
                currency: (backendPricing.standUpPouch.currency || normalizedCurrency)
                  .toString()
                  .toUpperCase(),
              },
            }
        ...
        try {
          ...
          if (referrer && referrer._id.toString() !== userId.toString()) {
            ...
            await referralService.createReferralRecord(
              ...
              roundAmount(pricingBreakdown.overall.discountedPrice)
            );
            logger.info(
