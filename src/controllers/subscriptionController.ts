...
/** Coerce DB/JSON date values for safe .getTime() usage after .lean() */
...
const daysUntilFromNow = (value: unknown): number | null => {
...
class SubscriptionController {
    ...
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      if (lang && typeof lang === 'string') {
        ...
        try {
          ...
          logger.warn("[getSubscriptions] failed to load user language, defaulting to en", {
            userId: req.user._id,
            error:
              userLangError instanceof Error
                ? userLangError.message
                : userLangError,
          });
        }
      }
      ...
      logger.debug("[getSubscriptions] query", {
      ...
      logger.debug("[getSubscriptions] fetched", {
        userId: userId.toString(),
      ...
      const formattedSubscriptions = subscriptions.map((sub: any) => {
        const daysUntilDelivery = daysUntilFromNow(sub.nextDeliveryDate);
        const daysUntilBilling = daysUntilFromNow(sub.nextBillingDate);
      ...
      logger.debug("[getSubscriptions] success", {
        userId: userId.toString(),
  ...
  getSubscriptionDetails = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const daysUntilDelivery = daysUntilFromNow(subscription.nextDeliveryDate);
      const daysUntilBilling = daysUntilFromNow(subscription.nextBillingDate);
