...
export class SubscriptionAutoRenewalService {
  ...
  async processRenewal(subscription: ISubscription): Promise<RenewalResult> {
    ...
    try {
      ...
      // Resolve currency from subscription pricing, then original order, then items, then default
      const subscriptionCurrency: string | null =
        (subscription as any).pricing?.overall?.currency ||
        (subscription as any).pricing?.sachets?.currency ||
        (subscription as any).pricing?.currency ||
        null;
      ...
      // Final currency resolution: subscription pricing → original order → fallback USD
      const resolvedCurrency: string =
        subscriptionCurrency ||
        (originalOrder as any)?.pricing?.overall?.currency ||
        (originalOrder as any)?.currency ||
        "USD";
