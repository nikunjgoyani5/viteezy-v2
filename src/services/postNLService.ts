...
export class PostNLService {
  ...
  async validateAddress(
    ...
    try {
      ...
      logger.debug("PostNL API v2 request", {
        url: urlWithParams.replace(this.apiKey, "[REDACTED]"),
        queryParams,
  ...
  private mapResponse(data: any): PostNLAddressValidationResult {
    if (Array.isArray(data)) {
      ...
      return {
        isValid: Boolean(hasUsefulData),
        normalizedAddress: hasUsefulData
          ? this.normalizeAddress(first)
          : undefined,
        raw: data,
        source: "postnl",
        reason: hasUsefulData
          ? undefined
          : "Address not found",
      };
    }
