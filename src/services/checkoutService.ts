...
import { resolveSingleCurrency } from "../utils/pricingCurrency";
...
class CheckoutService {
  ...
  async getCheckoutPageSummary(
    ...
    const sachetCartItemsPromises = sachetItems.map(async (item: any) => {
      ...
      const productPriceSource: ProductPriceSource = {
        price: {
          currency: basePlanPrice.currency,
    ...
    const resolvedCurrency = resolveSingleCurrency(
      cartItems.map((item: any) => item.basePlanPrice?.currency),
      "checkout summary"
    );
    ...
    if (sachetItems.length > 0) {
      ...
      if (selectedSachetPlan) {
        ...
        // subscriptionPlanDiscountAmount = actual saving vs base price for 90-day plans.
        // The 90-day 15% discount is already baked into sachetDiscountedPrice
        // (discountedPrice = basePrice * 0.85), so the real discount is simply
        // basePrice - discountedPrice, not a second 15% pass on the discounted value.
        if (
          selectedPlanDays === 90 &&
          !isOneTimePurchase &&
          sachetSubtotal > 0
          ...
          sachetSubscriptionPlanDiscountAmount = this.roundAmount(
            sachetSubtotal - sachetDiscountedPrice,
          );
        }
      } else {
    ...
    /// Calculate order amount for coupon validation (pre-tax, after plan + membership discounts).
    // Using pre-tax base keeps couponDiscountAmount semantically correct: it is a
    // discount on the product price, not on tax. grandTotal is unaffected because
    // couponDiscountAmount is later subtracted before tax is re-added.
    const orderAmountForCoupon = this.roundAmount(subtotalAfterDiscounts);
    ...
    /// Subtract coupon discount from pre-tax subtotal.
    // couponDiscountAmount was calculated on subtotalAfterDiscounts (pre-tax),
    // so this subtraction is consistent and does not touch the tax amount.
