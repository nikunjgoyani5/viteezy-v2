...
interface CreateProductData {
  ...
  ingredients: string[];
  ingredientMeta?: {
  sectionTitle?: { en: string; nl?: string; de?: string; fr?: string; es?: string };
  sectionSubtitle?: { en: string; nl?: string; de?: string; fr?: string; es?: string };
  backgroundImage?: { type?: string; url?: string; sortOrder?: number };
  excipients?: { en: string; nl?: string; de?: string; fr?: string; es?: string };
};
...
class ProductService {
  ...
  async updateProduct(
    ...
    if ((data as any).ingredientMeta !== undefined) {
      const rawExistingMeta: any = (existingProduct as any).ingredientMeta;
      const existingMeta: Record<string, any> =
        rawExistingMeta && typeof rawExistingMeta === "object"
          ? typeof rawExistingMeta.toObject === "function"
            ? rawExistingMeta.toObject()
            : JSON.parse(JSON.stringify(rawExistingMeta))
          : {};

      const incomingMeta = (data as any).ingredientMeta as Record<string, any>;

      // Shallow merge — only overwrite keys that are explicitly provided with a non-empty value
      ...
      updateData.ingredientMeta = mergedMeta;

      logger.info("[Update Product] Merged ingredientMeta to be saved", {
        productId,
        mergedIngredientMeta: mergedMeta,
      });
    }
