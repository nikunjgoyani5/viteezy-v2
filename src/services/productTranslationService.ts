...
export async function translateProduct(
  ...
  try {
    ...
    let translatedIngredientMeta = product.ingredientMeta;

    if (translatedIngredientMeta) {
      const [sectionTitle, sectionSubtitle, excipients] = await Promise.all([
        translatedIngredientMeta.sectionTitle
          ? googleTranslateService.translateText(
              translatedIngredientMeta.sectionTitle,
              translateCode
            )
        ...
        translatedIngredientMeta.sectionSubtitle
          ? googleTranslateService.translateText(
              translatedIngredientMeta.sectionSubtitle,
              translateCode
            )
        ...
        translatedIngredientMeta.excipients
          ? googleTranslateService.translateText(
              translatedIngredientMeta.excipients,   // ✅ FIX HERE
              translateCode
            )
          : Promise.resolve(""),
      ]);

      translatedIngredientMeta = {
        ...translatedIngredientMeta,
        sectionTitle,
        sectionSubtitle,
        excipients
    ...
    if (translatedStandupPouchPrice) {
      const pouchFields = ["count_0", "count_1"];
    ...
    const translatedProduct = {
      ...
      ingredientMeta: translatedIngredientMeta,
