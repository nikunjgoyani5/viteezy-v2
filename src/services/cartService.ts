...
class CartService {
  ...
  private async calculateCartTotalsWithVariantType(
    ...
    items.forEach((item: any) => {
      ...
      let taxRate = 0;
      let itemCurrency = item.price?.currency;
      ...
      if (itemVariantType === ProductVariant.SACHETS && product.sachetPrices) {
        ...
        if (thirtyDaysPlan) {
          ...
          itemCurrency = thirtyDaysPlan.currency || itemCurrency;
        } else {
        ...
        if (selectedCount) {
          ...
          itemCurrency = selectedCount.currency || itemCurrency;
          ...
          itemCurrency = standupPrice.currency || itemCurrency;
        }
      } else {
    ...
    return {
      ...
      currency: resolveSingleCurrency(currencies, "cart totals"),
    };
  }
  ...
  calculateTotalsFromCartItems(
    ...
    const currency = resolveSingleCurrency(
      items.map((item) => item?.price?.currency),
      "cart item totals"
    );
