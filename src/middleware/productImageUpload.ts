...
export const handleProductImageUpload = async (
  ...
  try {
    ...
    // Parse ingredientMeta from form-data if it is a JSON string
    if (
      req.body.ingredientMeta &&
      typeof req.body.ingredientMeta === "string"
    ) {
      try {
        req.body.ingredientMeta = JSON.parse(req.body.ingredientMeta);
      } catch (error) {
        logger.error(
          "[handleProductImageUpload] Failed to parse ingredientMeta",
          { ingredientMeta: req.body.ingredientMeta }
        );
      }
    }

    if (
      files?.["ingredientMeta.backgroundImage"] &&
      files["ingredientMeta.backgroundImage"].length > 0
    ) {
      const backgroundImageUrl = await fileStorageService.uploadFile(
        "products",
        files["ingredientMeta.backgroundImage"][0]
      );

      if (!req.body.ingredientMeta) {
        req.body.ingredientMeta = {};
      }

      req.body.ingredientMeta.backgroundImage = {
        type: "Image",
      ...
      logger.info(
        "[handleProductImageUpload] Uploaded ingredientMeta.backgroundImage",
        {
