...
const ContactUsBottom: React.FC = () => {
  ...
  const { data: settings, isLoading: settingsLoading } = useGeneralSettings(locale);
    ...
    <div className="container mx-auto px-4 pt-7 pb-12 lg:py-16">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col justify-between order-2 lg:order-1">
            ...
            {settingsLoading ? (
              ...
              <div className="space-y-3">
                {settings?.supportPhone && (
                  <div>
                    ...
                    <p className="text-base -mt-1 font-medium">
                      <a href={`tel:${settings.supportPhone.replace(/\s/g, "")}`} className="hover:text-teal-600">
                        {settings.supportPhone}
                      </a>
                    </p>
                  </div>
                ...
                {settings?.supportEmail && (
                  <div>
                    <p className="text-base font-medium">{t("emailLabel")}</p>
                    <p className="text-base -mt-1 font-medium">
                      <a href={`mailto:${settings.supportEmail}`} className="hover:text-teal-600">
                        {settings.supportEmail}
                      </a>
                    </p>
                  </div>
                ...
                {settings?.address && (
                  <div>
                    <p className="text-base font-medium">{t("headOffice")}</p>
                    <p className="text-base -mt-1 font-medium">
                      {settings.address}
                    </p>
                  </div>
          ...
          {/* Quiz CTA */}
          <div className="bg-sand-light/40 rounded-2xl p-4 md:p-2 flex flex-col md:flex-row items-center gap-4 md:gap-6 mt-12">
            <div className="relative w-full md:w-44 h-64 md:h-44 flex-shrink-0">
              <Image
                src="/contactUs.png"
                alt={t("productName")}
                fill
            ...
            <div className="text-left w-full md:w-auto">
              <h3 className="text-xl md:max-w-[85%] font-medium text-gray-900 mb-5">
                {t("readyToFeelBest")}
              </h3>
              <Link href="/quiz">
        ...
        <div className="rounded-2xl order-1 lg:order-2">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
              ...
              render={({ field }) => (
                <div className="w-full flex flex-col gap-1">
                  <textarea
                    ...
                    rows={5}
                    className={cn(
                      "w-full rounded-[12px] border bg-white px-4 py-3 text-sm text-gray-800 focus:outline-none focus:ring-2 resize-none placeholder:text-base h-36",
                      errors.message
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <div
                      className={`w-4 h-4 rounded border-2 transition-all duration-200 flex items-center justify-center ${field.value
            ...
            <button
              ...
              className="w-full bg-teal-green-color hover:bg-teal-green-color/85 text-white py-3 rounded-full transition-colors text-base cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
